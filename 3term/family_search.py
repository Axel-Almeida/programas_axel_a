...
def print_death_age(people_dict):
    print(f"Name and Age of death of the person.\n")
    for item in people_dict:
        d=int(f"{people_dict[item][3]}")
        b=int(f"{people_dict[item][2]}")
        age=d-b
        
        print(f"{people_dict[item][0]}: {age}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for item in marriages_dict:
        wedding=marriages_dict[item]
        husband=wedding[HUSBAND_KEY_INDEX]
        wife=wedding[WIFE_KEY_INDEX]
        year=wedding[WEDDING_YEAR_INDEX]
        h=people_dict.get(husband)
        w=people_dict.get(wife)
        hb=int(f"{people_dict[husband][2]}")
        ageh=year-hb
        wb=int(f"{people_dict[wife][2]}")
        agew=year-wb
        print(f"{h[0]} at the age of {ageh} and {w[0]} at the age of {agew} married in {year}.")
